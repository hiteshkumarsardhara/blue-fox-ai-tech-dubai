  ...
  return (
    <Container className="py-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">{t("portal.invest.title")}</h1>
          <p className="mt-1 text-muted">
            {t("portal.invest.subtitle")}
          </p>
        </div>
        <div className="rounded-xl border border-border bg-surface px-4 py-3">
          <p className="flex items-center gap-1.5 text-xs font-medium text-muted-2">
            <Wallet className="h-3.5 w-3.5" /> {t("portal.invest.availableBalance")}
          </p>
      ...
      {available === 0 && (
        <div className="mt-6 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-primary/30 bg-primary-soft px-4 py-3 text-sm">
          <span className="text-foreground">
            {t("portal.invest.emptyWallet")}
          </span>
          <Link href="/portal/deposit" className={buttonVariants({ size: "sm" })}>
            {t("portal.invest.depositFunds")} <ArrowRight className="h-4 w-4" />
          </Link>
        </div>
      )}
      ...
      {highestEver > 0 && (
        <div className="mt-6 flex items-start gap-2 rounded-xl border border-border bg-surface px-4 py-3 text-sm text-muted">
          ...
          <span>
            {t("portal.invest.highestPackagePrefix")}{" "}
            <span className="font-semibold text-foreground">{formatCents(highestEver)}</span>.{" "}
            {t("portal.invest.highestPackageSuffix")}
          </span>
        </div>
      )}
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-muted-2">{r.contractMonths} {t("portal.invest.months")}</span>
              </div>

              <h3 className="mt-3 text-base font-semibold text-foreground">{r.name}</h3>

              <div className="mt-4 rounded-xl border border-border bg-background/40 p-4">
                <p className="text-xs text-muted-2">{t("portal.invest.deposit")}</p>
              ...
              <dl className="mt-4 space-y-2.5 text-sm">
                <div className="flex items-center justify-between">
                  <dt className="flex items-center gap-1.5 text-muted">
                    <TrendingUp className="h-4 w-4 text-success" /> {t("portal.invest.monthlyReturn")}
                  </dt>
                ...
                <div className="flex items-center justify-between">
                  <dt className="flex items-center gap-1.5 text-muted">
                    <Clock className="h-4 w-4 text-primary" /> {t("portal.invest.totalOverTerm")}
                  </dt>
